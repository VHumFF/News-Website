...
export default function NewsListing() {
  ...
  const fetchCategoryArticles = async () => {
    ...
    try {
      ...
      const listType = filter === "trending" ? 1 : 0

      console.log("Category API call with listType:", listType, "filter:", filter)

      // Explicitly construct the URL with query parameters to ensure listType is included
      const response = await articlesApi.getByCategory(categoryId, {
        params: {
          listType: listType, // Explicitly set the parameter
          page,
          pageSize,
        },
      })
